describe('tests basic functionality of impacts', () => {
    it.skip('opens & checks date/name in detail window', () => {
        cy.login('QA_user', 'zIaNuhpGz8uxZRazhSCU') //zIaNuhpGz8uxZRazhSCU
        cy.wait(750)
        ...
        cy.wait(750)
        cy.openManagement('Management', 'BCM', 'Impacts')
    ...
    it('opens & checks date/name in detail window', () => {
        ...
        cy.switchTenant('cypressTenantProto')
        cy.wait(750)
        cy.openManagement('Management', 'BCM', 'Business Processes')
        /* ==== Generated with Cypress Studio ==== */
        cy.get('.p-button-label').first().click();;
        cy.get('.field--hidden > .p-inputtext').type('Testing BP', {delay:100});
        cy.get('.p-inputtextarea').type('description of BP', {delay:100});
        cy.get('.\\!bg-transparent').click();
        cy.get('[data-cy="assetBrowser_create"] > .p-button-label').click();
        /*
